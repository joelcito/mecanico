<div style="overflow-x: auto;">
    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_vehiculos" >
        <thead>
            <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                <th>Cliente</th>
                <th>Placa</th>
                <th>Marca</th>
                <th>Modelo</th>
                <th>Año</th>
                <th>Tipo</th>
                <th>Estado</th>
                <th>Actions</th>
            </tr>
        </thead>

        <tbody class="text-gray-600 fw-semibold">
            @forelse ($vehiculos as $vehiculo)
                <tr>
                    <td>
                        @if ($vehiculo->cliente && $vehiculo->cliente->user)
                            {{ $vehiculo->cliente->user->nombres }}
                            {{ $vehiculo->cliente->user->ap_paterno }}
                            {{ $vehiculo->cliente->user->ap_materno }}

                        @else
                         -
                        @endif
                    </td>
                    <td>
                        <span class="fw-bold">
                            {{ $vehiculo->placa }}
                        </span>
                    </td>
                    <td>
                        {{ $vehiculo->marca->nombre ?? '-' }}
                    </td>
                    <td>
                        {{ $vehiculo->modelo }}
                    </td>
                    <td>
                        {{ $vehiculo->anio ?? '-' }}
                    </td>
                    <td>
                        @if ($vehiculo->tipo_vehiculo == 'AUTOMOVIL')

                            <span class="badge badge-light-primary">
                                Automóvil
                            </span>

                        @else

                            <span class="badge badge-light-warning">
                                Motocicleta
                            </span>

                        @endif
                    </td>
                    <td>

                        @if ($vehiculo->estado == 'ACTIVO')
                            <span class="badge badge-light-success">
                                Activo
                            </span>

                        @else
                            <span class="badge badge-light-danger">
                                Inactivo
                            </span>

                        @endif
                    </td>
                    <td>
                        <button
                            class="btn btn-icon btn-sm btn-warning btn-circle"
                            title="Editar vehículo"
                            onclick='editarVehiculo(@json($vehiculo))' >
                            <i class="fa fa-edit"></i>
                        </button>
                        <button
                            class="btn btn-icon btn-sm btn-danger btn-circle"
                            title="Eliminar vehículo"
                            onclick='eliminarVehiculo(
                                "{{ $vehiculo->id }}",
                                "{{ $vehiculo->placa }}")'  >
                            <i class="fa fa-trash"></i>
                        </button>
                    </td>
                </tr>
            @empty
                       <h4>No hay vehículos registrados</h4>
            @endforelse

        </tbody>
    </table>
</div>

<script>
    $(document).ready(function () {
        $('#kt_table_vehiculos').DataTable({
            lengthMenu: [10, 25, 50, 100],
            dom: '<"dt-head row"<"col-md-6"l><"col-md-6"f>><"clear">t<"dt-footer row"<"col-md-5"i><"col-md-7"p>>',
            language: {
                paginate: {
                    first: 'Primero',
                    last: 'Último',
                    next: 'Siguiente',
                    previous: 'Anterior'
                },
                search: 'Buscar:',
                lengthMenu: 'Mostrar _MENU_ registros por página',
                info: 'Mostrando _START_ a _END_ de _TOTAL_ registros',
                emptyTable: 'No hay datos disponibles'
            },
            order: [],
            responsive: true
        });

    });
</script>